...
func generateGoStub(matches []string, outFile string, funcname string) error {
	...
	defer f.Close()
	fmt.Fprintln(f, "//go:build dll && windows")
	fmt.Fprintln(f, "// Code generated by goppeling; DO NOT EDIT.")
...
func main() {
	...
	outDir := flag.String("dir", ".", "Output Go dir for stubs")
	flag.Parse()

	if *dllPath == "" && *exePath == "" {
		log.Fatalf("Usage: %s [-dll <dllpath>] [-exe <exepath>] [-dir <outdir>]", os.Args[0])
	}
	...
	if *dllPath != "" && *exePath != "" {
		...
		fmt.Println("[*] Please type the name of the function you would like to hijack")
		fmt.Scanln(&nameOfFunc)

		fmt.Printf("[+] Generating Go no-op stub file: %s/generate_windows.go\n", *outDir)
		if err := generateGoStub(matches, filepath.Join(*outDir, "generated_windows.go"), nameOfFunc); err != nil {
